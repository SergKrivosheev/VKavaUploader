    ...
    def get_photos(self, id, photos_quantity):
        ...
        for item in tqdm(req['response']['items']):
            ...
            for photo in item['sizes']:
                if photo['type'] == 'z' or photo['type'] == 'w' or photo['type'] == 'y' or photo['type'] == 'x': #т.к. у меня всего одна аватарка с размером z поэтому я выбираю просто все большие
                    if photo_name not in dict_photos:
                        dict_photos[photo_name] = [photo['url'], photo['type'], item['date']]
                        print(dict_photos)
                        break
                    else:
...
class YaUploader:
    ...
    def upload_json(self, dictionary, file_path):
        for item in tqdm(dictionary):
            json_file = [{"file name": f"{item}.jpg", "size": f"{dictionary[item][1]}"}]
            with open(f'{item}.json', 'w') as outfile:
                json.dump(json_file, outfile)
            href = self.get_upload_link(file_path=f'{file_path}/{item}.json').get("href", "")
    ...
    def upload_photos(self, dictionary, file_path):
        ...
        for item in tqdm(dictionary):
            ...
            href = self.get_upload_link(file_path=f'{file_path}/{item}.jpg').get("href", "")
            response = requests.put(href, data=r.content)
            response.raise_for_status()
            if response.status_code == 201:
                print("Success")
                time.sleep(1)
        self.upload_json(dictionary, file_path)  # выполняем загрузку json файлов, как аргумент используем словарь
...
if __name__ == '__main__':
    ...
    #with open('tokens.txt', 'r') as f: # простой способ чтения токенов из одного файла
        #VKTOKEN = f.readline()
        #YATOKEN = f.readline()
    ...
    uploader.upload_photos(downloader.get_photos(vk_user_id, int(photos_quantity)), dir_name)